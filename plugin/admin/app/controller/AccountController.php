...
class AccountController extends Crud
{
     ...
     * @var string[]
     */
    protected $noNeedLogin = ['login', 'logout', 'captcha','getcaptcha'];
    ...
    public function login(Request $request): Response
    {
        ...
        if (strtolower($captcha) !== session('captcha-login')) {
            return $this->json(2, '验证码错误',['captcha'=>session('captcha-login')]);
        }
    ...
    public function getcaptcha(Request $request){
        return $this->success('',['captcha'=>session('captcha-login')]);
    }
    /**
     * 退出
     * @param Request $request
